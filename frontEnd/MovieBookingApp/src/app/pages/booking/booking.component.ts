...
export class BookingComponent {
  ...
  public onToggle(i:number,value:string): void {
    // Switch between red and green.Tracked the toggled state of elements.If previously togged on (green) so it toggled to toggled-off state(red)and temp array hole the value of the toggled element.
    if(this.clicked[i]== true){ 
      this.clicked[i] = false;
      this.temp[i] = "$"
    }
    //if conditions is false(toggled off i.e. red) then it set toggled-on state(green).after that it checks whether temp array already contains the value or not.if it doesn't then we set the value parameter.
    else{
      this.clicked[i] = true;
